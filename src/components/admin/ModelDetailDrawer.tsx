...
export function ModelDetailDrawer({ model, open, onOpenChange, onSave }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Track what admin manually edited as overrides
  ...
  const ratiosStr = (form.supported_ratios || []).join(', ');
  const sizesStr = (form.supported_sizes || []).join(', ');
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={e => setForm(f => ({ ...f, cost_per_run: parseFloat(e.target.value) || 0 }))}
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1">
                {(form.supported_ratios || []).map(r => (
                  <Badge key={r} variant="outline" className="text-[10px] bg-muted/20">
                    {r}
                    <button className="ml-1 hover:text-destructive" onClick={() => setForm(f => ({ ...f, supported_ratios: (f.supported_ratios || []).filter(x => x !== r) }))}>
                      <X size={8} />
                    </button>
                  </Badge>
                ))}
              ...
              <Input
                placeholder="Add ratio (e.g. 21:9) and press Enter"
                className="h-7 text-[11px] mt-1"
                onKeyDown={e => {
                  if (e.key === 'Enter' && (e.target as HTMLInputElement).value.trim()) {
                    const v = (e.target as HTMLInputElement).value.trim();
                    setForm(f => ({ ...f, supported_ratios: [...(f.supported_ratios || []), v] }));
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1">
                {(form.supported_sizes || []).map(s => (
                  <Badge key={s} variant="outline" className="text-[10px] bg-muted/20">
                    {s}
                    <button className="ml-1 hover:text-destructive" onClick={() => setForm(f => ({ ...f, supported_sizes: (f.supported_sizes || []).filter(x => x !== s) }))}>
                      <X size={8} />
                    </button>
                  </Badge>
                ))}
              ...
              <Input
                placeholder="Add size preset (e.g. square_hd)"
                className="h-7 text-[11px] mt-1"
                onKeyDown={e => {
                  if (e.key === 'Enter' && (e.target as HTMLInputElement).value.trim()) {
                    const v = (e.target as HTMLInputElement).value.trim();
