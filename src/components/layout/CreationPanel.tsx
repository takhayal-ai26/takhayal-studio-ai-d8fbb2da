...
const RESOLUTIONS = [
  ...
  { label: '2K', value: '2K', desc: 'High quality' },
      ...
      <div className="flex-1 overflow-y-auto overflow-x-visible p-4 space-y-3 scrollbar-thin">
        ...
        <div className="rounded-2xl bg-card/80 backdrop-blur-sm p-4 border border-border/10">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium transition-all duration-150 ${
                  enhancePrompt
                    ...
                    : 'bg-muted/30 text-muted-foreground hover:text-foreground border border-transparent hover:border-border/20'
                }`}
              >
                <Sparkles size={11} className={enhancePrompt ? 'text-primary' : ''} />
                Enhance
              </button>
            </div>
          </div>
          <textarea
            ...
            className="w-full min-h-[130px] bg-background/80 border border-border/15 rounded-xl p-3.5 text-[13px] text-foreground placeholder:text-muted-foreground/30 focus:border-primary/50 focus:outline-none resize-none leading-relaxed transition-colors"
          />
        ...
        <button className="w-full rounded-2xl bg-card/50 border border-dashed border-border/15 p-5 flex flex-col items-center justify-center gap-2 text-muted-foreground hover:text-foreground hover:border-primary/30 hover:bg-primary/[0.03] transition-all duration-200 group">
          <div className="w-10 h-10 rounded-xl bg-primary/[0.08] flex items-center justify-center group-hover:bg-primary/15 transition-colors">
            <Upload size={18} className="text-primary/70 group-hover:text-primary transition-colors" />
          </div>
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${
                openDropdown === 'model' ? 'bg-primary/15' : 'bg-primary/[0.08]'
              }`}>
          ...
          {openDropdown === 'model' && (
            <div className="absolute left-0 right-0 bottom-full mb-2 bg-card border border-border/20 rounded-2xl p-2 shadow-2xl shadow-black/40 z-50 animate-fade-in max-h-[320px] overflow-y-auto">
              <p className="text-[10px] text-primary/40 uppercase tracking-wider font-medium px-3 pt-2 pb-2 flex items-center gap-1.5">
                <Cpu size={10} />
                Select model
              </p>
              {MODELS.map(m => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-left flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground/50">{m.desc}</p>
                    </div>
          ...
          <div className="relative flex-1">
            ...
            >
              <div className="flex items-center gap-2.5">
                <Maximize size={14} className="text-primary/70" />
                <div className="text-left">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-150 ${
                        isActive ? 'bg-primary/10 text-primary' : 'text-foreground hover:bg-muted/10'
                      }`}
                    >
                      <span>{s.label}</span>
                      <div className="flex items-center gap-2">
                        <span className={`text-[14px] ${isActive ? 'text-primary/60' : 'opacity-30'}`}>{s.icon}</span>
                ...
                {RESOLUTIONS.map(r => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span>{r.label}</span>
                        <span className={`text-[10px] ${isActive ? 'text-primary/50' : 'text-muted-foreground/40'}`}>{r.desc}</span>
                      </div>
                      {isActive && <Check size={13} className="text-primary" />}
                    </button>
                  );
        ...
        <button
          ...
          className={`w-full h-14 rounded-2xl text-[15px] font-medium transition-all duration-150 flex items-center justify-center gap-3 ${
            canGenerate
              ...
              : 'bg-card border border-border/10 text-muted-foreground cursor-not-allowed'
          }`}
        >
          {isGenerating ? (
