...
export function RightPanel() {
  const { prompt, setPrompt, selectedTemplate, setSelectedTemplate, selectedStyle, setSelectedStyle, aspectRatio, setAspectRatio, quality, setQuality, enhancePrompt, setEnhancePrompt, generate, isGenerating, credits, getCreditCost } = useApp();
  const { t } = useLanguage();
  ...
  const handleTemplateSelect = (tpl: string) => { if (selectedTemplate === tpl) { setSelectedTemplate(null); } else { setSelectedTemplate(tpl); setPrompt(TEMPLATE_PROMPTS[tpl]); } };
    ...
    <aside className="hidden lg:flex flex-col w-[320px] border-l border-surface-border/50 bg-background flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto pb-[160px]">
        ...
        <Section title={t.studio.format}>
          ...
          <div className="flex gap-2">{ratios.map(r => (<button key={r} onClick={() => setAspectRatio(r)} className={`flex-1 h-9 rounded-lg text-[13px] font-medium transition-colors ${aspectRatio === r ? 'bg-primary text-primary-foreground' : 'bg-card border border-surface-border text-muted-foreground hover:text-foreground'}`}>{r}</button>))}</div>
          <p className="text-[12px] text-muted-foreground mb-2.5 mt-3.5">{t.studio.quality}</p>
          <div className="flex gap-2">{(['standard', 'hd'] as Quality[]).map(q => (<button key={q} onClick={() => setQuality(q)} className={`flex-1 h-9 rounded-lg text-[13px] font-medium transition-colors flex items-center justify-center gap-2 ${quality === q ? 'bg-primary text-primary-foreground' : 'bg-card border border-surface-border text-muted-foreground hover:text-foreground'}`}>{q === 'standard' ? t.studio.standard : t.studio.hd}{q === 'hd' && quality !== 'hd' && (<span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/[0.12] text-primary">+4 {t.toolPage.credits}</span>)}</button>))}</div>
        </Section>
      </div>
