...
function ModelList({ models, selectedModelId, language, onSelect }: { models: ModelRecord[]; selectedModelId: string; language: string; onSelect: (id: string) => void }) {
  return (
    <>
      {models.map((m, i) => {
        ...
        return (
          ...
          >
            <div className="flex-1 min-w-0">
              <p className="text-[15px] font-medium leading-tight" style={{ color: isActive ? 'hsl(var(--primary))' : 'hsl(var(--foreground))' }}>
                {m.model_name}
              </p>
              {bestFor && <p className="text-[12px] mt-0.5 truncate text-muted-foreground">{bestFor}</p>}
            </div>
      ...
      {models.length === 0 && (
        <p className="text-[13px] text-center py-8 text-muted-foreground">No active models</p>
      )}
    </>
  );
}
...
export function ModelDropdown({ models, selectedModelId, language, anchorRect, onSelect, onClose }: Props) {
  ...
  if (isMobile) {
    return (
      <MobileBottomSheet title="Select Model" open onClose={onClose}>
        <ModelList models={models} selectedModelId={selectedModelId} language={language} onSelect={onSelect} />
      </MobileBottomSheet>
    );
  }
