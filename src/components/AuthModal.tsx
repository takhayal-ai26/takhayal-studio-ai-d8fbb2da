...
export function AuthModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="absolute inset-0 bg-black/60 animate-fade-in" onClick={closeAuthModal} />
      <div className="relative w-full max-w-[420px] rounded-2xl bg-card border border-border p-10 animate-scale-in">
        <button onClick={closeAuthModal} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-muted/20 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
          <X size={16} />
        </button>
        ...
        <div className="text-center mb-7">
          ...
          <h2 className="text-[22px] font-medium text-foreground leading-tight">{t.headline}</h2>
        ...
        {view === 'forgot' ? (
          ...
          <form onSubmit={handleForgotPassword} className="space-y-3">
            ...
            <button type="submit" disabled={loading}
              className="w-full h-12 rounded-[10px] text-[15px] font-medium text-white transition-all disabled:opacity-60"
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-3">
            ...
            <div>
              ...
              {tab === 'login' && (
                <button type="button" onClick={() => { setView('forgot'); setError(''); setSuccess(''); }}
                  className="text-[12px] text-muted-foreground hover:text-foreground transition-colors mt-1 float-right">{t.forgot}</button>
              )}
            </div>
            {tab === 'signup' && (
            ...
            {success && <p className="text-[13px] text-green-500">{success}</p>}
            <button type="submit" disabled={loading}
              className="w-full h-12 rounded-[10px] text-[15px] font-medium text-white transition-all disabled:opacity-60 hover:brightness-90 mt-1"
              style={{ backgroundColor: 'hsl(var(--cta-primary))' }}>
              {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (tab === 'login' ? t.login : t.createAccount)}
            </button>
          </form>
        ) : (
          /* Main view with Google + email option */
          <div className="space-y-3">
            <button onClick={handleGoogle} disabled={loading}
              className="w-full h-12 rounded-[10px] border border-border bg-card text-foreground text-sm font-medium flex items-center justify-center gap-3 hover:bg-muted/10 transition-all disabled:opacity-60">
            ...
            <button onClick={() => setView('email')} disabled={loading}
              className="w-full h-12 rounded-[10px] border border-border bg-card text-foreground text-sm font-medium flex items-center justify-center gap-3 hover:bg-muted/10 transition-all disabled:opacity-60">
              <Mail size={17} />
              {t.emailBtn}
            </button>
