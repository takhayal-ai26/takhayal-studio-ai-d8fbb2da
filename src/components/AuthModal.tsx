        ...
        <div className="w-full lg:w-[46%] flex flex-col p-8 lg:p-10 overflow-y-auto">
          ...
          <div className="flex p-1 rounded-full bg-[hsl(0,0%,10%)] mb-7 w-fit">
            {(['signup', 'login'] as const).map(tb => (
              <button key={tb} onClick={() => { setTab(tb); setShowEmailForm(false); setShowForgotPassword(false); setError(''); setSuccessMessage(''); }} className={`px-6 py-2 rounded-full text-[13px] font-medium transition-all duration-200 ${tab === tb ? 'bg-primary text-primary-foreground' : 'text-[hsl(0,0%,42%)] hover:text-foreground'}`}>
          ...
          {successMessage ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
              ...
              <p className="text-[13px] text-[hsl(0,0%,42%)] leading-relaxed max-w-[280px]">{successMessage}</p>
            </div>
          ) : showForgotPassword ? (
            <form onSubmit={handleForgotPassword} className="space-y-3.5 flex-1">
              <button type="button" onClick={() => { setShowForgotPassword(false); setError(''); }} className="text-[12px] text-[hsl(0,0%,42%)] hover:text-foreground transition-colors mb-1">{t.auth.backToOptions}</button>
            ...
            <div className="space-y-3 flex-1">
              <button onClick={handleGoogle} disabled={loading} className="w-full h-[52px] rounded-[14px] border border-[hsl(0,0%,16%)] bg-transparent text-foreground text-sm font-medium flex items-center justify-center gap-3 hover:bg-[hsl(0,0%,10%)] hover:border-[hsl(0,0%,22%)] transition-all duration-150 disabled:opacity-50">
              ...
              <button onClick={handleApple} disabled={loading} className="w-full h-[52px] rounded-[14px] border border-[hsl(0,0%,16%)] bg-transparent text-foreground text-sm font-medium flex items-center justify-center gap-3 hover:bg-[hsl(0,0%,10%)] hover:border-[hsl(0,0%,22%)] transition-all duration-150 disabled:opacity-50">
