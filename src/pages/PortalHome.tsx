        ...
        <section className="mb-12">
          ...
          <div className="columns-2 md:columns-3 lg:columns-4 gap-1.5 [column-fill:_balance]">
            {filteredTemplates.map((tpl) => (
              <button key={tpl.id} onClick={() => goToCanvas(tpl.prompt, tpl.name)} className="group w-full rounded-2xl overflow-hidden break-inside-avoid mb-2 block text-left hover:shadow-lg hover:shadow-black/10 transition-shadow duration-300">
                <div className="relative overflow-hidden" style={{ aspectRatio: ratioToNumber(tpl.ratio) }}>
                  ...
                  <h3 className="absolute bottom-2.5 left-3 right-3 text-sm font-semibold text-white leading-tight drop-shadow-md">{tpl.name}</h3>
