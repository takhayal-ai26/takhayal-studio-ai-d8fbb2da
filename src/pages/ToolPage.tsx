...
export default function ToolPage() {
  ...
  const { requireAuth, credits } = useApp();
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="pt-16 flex items-center justify-center h-screen">
          <div className="text-center">
            <h1 className="text-2xl font-light text-foreground mb-2">Tool not found</h1>
            <button onClick={() => navigate('/home')} className="text-primary text-sm hover:underline">
              Back to Home
            </button>
          </div>
        </div>
      ...
      <div className="pt-16">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-8 flex items-center min-h-[480px]">
              ...
              <div className="backdrop-blur-xl bg-card/60 border border-border rounded-2xl p-6 md:p-8">
                ...
                {tool.inputType === 'prompt' ? (
                  ...
                  <div className="w-full h-32 border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center gap-2 hover:border-primary/50 transition-colors cursor-pointer bg-background/40">
                    ...
                    <span className="text-[13px] text-muted-foreground">Drop your image here</span>
                ...
                <button
                  ...
                  disabled={tool.inputType === 'prompt' && !inputValue.trim()}
                ...
                >
                  <span>{tool.inputType === 'prompt' ? 'Generate' : 'Upload & Process'}</span>
            ...
            <div className="hidden lg:flex flex-1 items-center justify-center pl-12">
              <div className="text-right max-w-xs">
                ...
                <p className="text-[13px] text-muted-foreground mt-3">Powered by state-of-the-art AI</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-5 md:px-8 py-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[15px] font-medium text-foreground">Examples</h2>
            <span className="text-[12px] text-muted-foreground">{tool.examples.length} examples</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {tool.examples.map((ex, i) => (
              <button
                ...
                className="group relative rounded-xl overflow-hidden border border-border hover:border-primary transition-all hover:scale-[1.02]"
              >
        ...
        <section className="max-w-7xl mx-auto px-5 md:px-8 pb-16 text-center">
          <div className="py-12 rounded-2xl border border-border bg-card/30">
            ...
            <h2 className="text-xl font-light text-foreground mb-2">Start creating now</h2>
            <p className="text-[13px] text-muted-foreground mb-6">No setup required. Jump in and create.</p>
            <button
              onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
              className="h-11 px-6 rounded-xl bg-primary text-primary-foreground text-[13px] font-medium hover:opacity-90 transition-opacity"
            >
              Try {tool.name}
            </button>
          </div>
